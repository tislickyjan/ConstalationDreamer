...
class ConstalationDrawer:
    ...
    def __init__(self):
        #TODO vice rozliseni nez jen FullHD
    ...
    def DrawPlanet(self, planet):
        ...
        if planet.rings is not None:
            ...
            a, b = self.CalculateBounds(planet.position, planet.size*scaleFactor)
            self.drawPlace.arc((a[0], a[1], b[0], b[1]), start=180, end=360, fill=planet.rings[0], width=ringSize)
            self.drawPlace.ellipse(planet.box, fill=planet.color[0])
        ...
        if planet.moons is not None:
            ...
            moonPosition = np.array((np.cos(t),np.sin(t)))*moonFac + planet.position
    ...
    def DrawAsteroidField(self, field, angle):
        ...
        for i in corrMod:
            ...
            for t in asteroidPositions:
                ...
                astPosition = np.array((field.position[0] * np.cos(t), field.position[1] * np.sin(t))) + field.size
